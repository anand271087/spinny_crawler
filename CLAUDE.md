...
| **Part-level grain for Mahindra/MG** (2026-06-13): drill into each assembly via `GetIllustrationPartsJQ` for per-part Description + Start/End Date + part_structure. `item_code`=partNo (not figno). Mahindra relies on a **fresh-login hard-reset** between categories (deep drill corrupts the SPA session; one quick re-nav attempt then reset — don't restore multi-retry). | Spinny scope extension. Verified full runs: Mahindra 28,222/6.0h, MG 10,281/99min, Toyota +description/dates 26,820/12.8min. | per_site_notes §V2.3/V2.4/V2.5 |
...
7. **MRP is NOT in `/pages/parts/`**: requires a separate picklist API call (queued, not yet wired). Rows currently ship as `crawl_status=partial` with item_code/name/compat fully populated.
   - **Toyota extra fields (2026-06-13)**: `description` (= partItems description), `start_date` (dynamicColumns `code=FDATE`), `end_date` (dynamicColumns `code=TDATE`) are extracted via `_dyn_col()` — all already in the `/pages/parts/` response, zero extra calls. `start_date`/`end_date` are bonus (not required), so a blank TDATE doesn't add `partial`.
...
7. **PART-LEVEL drill (2026-06-13, default ON)**: the assembly grid is NOT the leaf — Spinny needs per-part Description + Start/End Date, which live one level deeper. `_drill_assemblies` clicks each assembly → auto-accepts the native "search parts with prod date" confirm (`page.on("dialog", accept)` + `_accept_confirm` for the modal variant) → parses `POST /webapi/api/Illustration/GetIllustrationPartsJQ`. Clicking an assembly HIDES the grid, so between assemblies click the section breadcrumb to re-render it. Toggle with `MAHINDRA_PART_LEVEL`/`MG_PART_LEVEL` (=0 → old assembly grain); cap runtime with `*_MAX_ASSEMBLIES`.
8. **Field map (part grain)**: `item_code ← partNo`, `item_name`/`description ← part description`, `start_date ← startDate`, `end_date ← endDate` (blank for in-production parts — NOT required, so empty doesn't flag `partial`; per-brand CSV drops the all-empty column), `part_structure ← cat > variant > section > assembly_name (figno)`. MRP not required per xlsx.
9. **Mahindra needs the hard-reset; MG does not.** Deep part-drilling corrupts Mahindra's SPA session so PV-root re-nav fails on most category transitions — `crawl()` falls back to `open_session()` (drop BrowserContext + fresh login). Keep it at ONE quick re-nav attempt then hard-reset; do NOT restore the old 3-retry loop (retries reliably fail post-drill). MG's `FIGURE SEARCH` + `page.goto` model reset survives the drill, so MG runs clean without resets. Verified full runs 2026-06-13: Mahindra 28,222 parts / 6.0h (14 hard-resets, 0 aborts), MG 10,281 / 99 min, Toyota 26,820 / 12.8 min.
